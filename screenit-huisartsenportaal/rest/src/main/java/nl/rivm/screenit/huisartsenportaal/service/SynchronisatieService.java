...
public interface SynchronisatieService
{
	...
	void syncHuisarts(Huisarts huisarts);

	void syncAanvraag(LabformulierAanvraag aanvraag);

	void syncLocatie(Huisarts huisarts, Locatie locatie, Boolean herzendVerificatieMail);

	HuisartsDto getHuisartsDto(Huisarts huisarts);

	void herzendVerificatieMail(Huisarts huisarts, Locatie locatie);
