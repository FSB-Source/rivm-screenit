...
angular.module('rivmUistrijkendarts').controller('wijzigenGegevensCtrl', function ($http, $scope, $routeParams, $rootScope, $location, NgTableParams, userfactory, locatiefactory,
																				   toaster, registratiefactory, overeenkomstfactory, formulierenfactory, verificatiefactory) {
	...
	var ctrl, wijzigGegevens, huisarts, locatie, memoryLocatie, locaties, wachtwoord, hasLocatie, hasActieveLocatie;
		...
		ctrl.tableParams = new NgTableParams({
			...
			getData: function (params) {
				...
				return locatiefactory.getLocaties(ctrl.locatieZoekObject).$promise.then(function (data) {
					...
					ctrl.hasLocatie = data.locaties.size > 0;
					let aantalActieveLocaties = 0
					for (locatie in data.locaties) {
						if (locatie.status !== 'INACTIEF') {
							aantalActieveLocaties++
						}
					}
					ctrl.hasActieveLocatie = aantalActieveLocaties > 0;
					ctrl.locatieTitel = aantalActieveLocaties > 1 ? "2. Locaties" : "2. Locatie"
