...
public class HuisartsportaalLoggingFilter implements Filter
{

	private static final Logger LOG = LoggerFactory.getLogger(HuisartsportaalLoggingFilter.class);
	...
	private boolean doesNotContainPassword(HttpServletRequest httpServletRequest, BufferedRequestWrapper bufferedReqest, BufferedResponseWrapper bufferedResponse)
    {
		try
        {
			return !(httpServletRequest.getRequestURI().contains("wachtwoord")
					|| bufferedReqest.getRequestBody().contains("wachtwoord")
					|| bufferedResponse.getContent().contains("wachtwoord"));
		}
		catch (Exception e)
        {
			e.printStackTrace();
