...
@RestController
@RequestMapping("/api/v1/betaling")
...
public class BetalingController extends BaseController
{
	...
	@PreAuthorize("hasRole('ROLE_AANVRAGEN')")
	public ResponseEntity getHuidigeVerrichtingenCsv(@Valid @RequestBody BetalingZoekObjectDto betalingZoekObjectDto, BindingResult result)
	{
		if (result.hasErrors())
		{
			return ResponseEntity.badRequest().body(result.getAllErrors());
		}
		List<BetalingCsvDto> betalingCsvDtos = betalingService.getBetalingenCsv(getIngelogdeHuisarts(), betalingZoekObjectDto);
		return new ResponseEntity(betalingCsvDtos, HttpStatus.OK);
	}
}
