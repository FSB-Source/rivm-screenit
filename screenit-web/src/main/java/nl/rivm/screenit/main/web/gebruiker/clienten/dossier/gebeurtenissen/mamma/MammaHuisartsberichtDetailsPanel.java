...
public class MammaHuisartsberichtDetailsPanel extends AbstractGebeurtenisDetailPanel
{
	...
	public MammaHuisartsberichtDetailsPanel(String id, IModel<ScreeningRondeGebeurtenis> model)
	{
		super(id, model);
	}
	...
	private WebMarkupContainer maakHuisartsBerichtContainer(boolean visible)
	{
		...
		container.add(new Label("mammaHuisartsBericht.berichtType.naam"));
		container.add(new Label("mammaHuisartsBericht.aanmaakDatum"));
	...
	private HuisartsZoekenPanel maakHuisartsZoekenPanel()
	{
		HuisartsZoekenPanel result = new HuisartsZoekenPanel("huisartsZoeken", false)
		{
			...
			{
				MammaHuisartsberichtDetailsPanel.this.onHuisartsGekozen(huisarts);
	...
	private String getStandardButtonText()
	{
		...
		if (getHuisartsBericht().getStatus() == MammaHuisartsBerichtStatus.VERSTUREN_MISLUKT)
		{
	...
	{
		return selectedHuisarts.getObject();
	}

	private MammaHuisartsBericht getHuisartsBericht()
	{
		return getModelObject().getMammaHuisartsBericht();
	}
