...
import nl.rivm.screenit.main.web.gebruiker.clienten.contact.mamma.MammaAfspraakKiezenPanel;
...
public class ClientMammaAfspraakAanmakenPage extends ClientBasePage
{
	...
	public ClientMammaAfspraakAanmakenPage(IModel<Client> clientModel)
	{
		Form<Void> form = new Form<>("form");
		add(form);
		final MammaAfspraakKiezenPanel afspraakKiezenPanel = new MammaAfspraakKiezenPanel("afspraakAanmaken", clientModel, false);
		form.add(afspraakKiezenPanel);
		form.add(new IndicatingAjaxSubmitLink("opslaan", form)
		{
			...
			public void onSubmit(AjaxRequestTarget target)
			{
				...
				if (!form.hasError())
				{
					Map<ExtraOpslaanKey, Object> opslaanObjecten = afspraakKiezenPanel.getOpslaanObjecten();
					ClientContact contact = new ClientContact();
					ClientContactActie actie = new ClientContactActie();
					actie.setType(ClientContactActieType.MAMMA_AFSPRAAK_MAKEN);
					...
					extraOpslaanObjecten.put(ClientContactActieType.MAMMA_AFSPRAAK_MAKEN, opslaanObjecten);
					try
					{
						clientContactService.saveClientContact(contact, extraOpslaanObjecten, ingelogdeClient);
						ScreenitSession.get().success(getString("success.afspraak_aangemaakt"));
					}
		...
		form.add(new IndicatingAjaxLink<MammaAfspraak>("uitstellen")
		{
			...
			{
				setResponsePage(new ClientMammaUitstellenPage(afspraakKiezenPanel.getModel()));
			}
		}.setVisible(afspraakService.magUitstellen(clientModel.getObject().getMammaDossier())));
	...
	{
		return new SimpleStringResourceModel("label.afspraakmaken");
	}
