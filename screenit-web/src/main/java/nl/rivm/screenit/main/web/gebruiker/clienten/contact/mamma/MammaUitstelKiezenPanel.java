	...
	public MammaUitstelKiezenPanel(String id, IModel<Client> clientModel)
	{
		...
		uitstelZoekenPanel = new MammaUitstelZoekenPanel("uitstelZoekenPanel", clientModel)
		{
			...
			protected void nieuwUitstel(AjaxRequestTarget target, IModel<MammaStandplaatsPeriodeMetAfstandDto> model, Date zoekDatum)
			{
				...
				MammaAfspraak laatsteAfspraak = screeningRonde.getLaatsteUitnodiging().getLaatsteAfspraak();
				...
				if (laatsteUitstel != null && laatsteUitstel.getUitnodiging() == null && laatsteUitstel.getGeannuleerdOp() == null)
				{
					...
					uitstelModel = ModelUtil.cModel(laatsteUitstel);
					MammaUitstel uitstel = uitstelModel.getObject();
					uitstel.setStreefDatum(zoekDatum);
					uitstel.setStandplaats(standplaats);
				}
				else
				{
					MammaUitstel uitstel = baseFactory.maakUitstel(screeningRonde, standplaats, zoekDatum, MammaUitstelReden.CLIENT_CONTACT);
