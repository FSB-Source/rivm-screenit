...
public class ClientColonDashboardPage extends ClientBasePage
{
	...
	public ClientColonDashboardPage()
	{
		Boolean toonVervangendeTekst = preferenceService.getBoolean(PreferenceKey.COLON_CLIENTPORTAAL_TOON_VERVANGENDE_TEKST.name(), Boolean.FALSE);
		if (Boolean.TRUE.equals(toonVervangendeTekst))
		{
			throw new RestartResponseException(ClientColonDashboardVervangendeTekstPage.class);
		}
		...
		add(new AfsprakenPanel("afspraken", client));
		add(new ClientHistoryPanel("history", client)
		{
			...
			{
				return clientGebeurtenisService.getClientColonGebeurtenissen(ModelProxyHelper.deproxy(modelClient.getObject()));
			}

		});
		add(new ColonOverigeDashboardActiesPanel("overige", client));
