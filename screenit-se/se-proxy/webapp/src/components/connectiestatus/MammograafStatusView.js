...
export type MammograafStatusViewProps = {
...
export default class MammograafStatusView extends React.Component<MammograafStatusViewProps> implements ConnectieStatusItem {

    constructor(props: MammograafStatusViewProps) {
        super(props);
        this.props = props;
            ...
            <Col md={12}>
                ...
                <Row className={'connectiestatus-leaf'}>
                    <Col>
                        ...
                        <MammograafDicomBerichtStatusView messageType={'MPPS'} level={this.getMppsStatusLevel()} timestamp={mammograafStatus.laatsteSuccesMppsBerichtTimestamp}
                                                          onClick={() => this.props.toonDicomFouten('MPPS', mammograafStatus)}/>
                    </Col>
                </Row>
                <Row className={'connectiestatus-leaf'}>
                    <Col>
                        <i className={connectieStatusLevels[this.getDmwlStatusLevel()]}/>
                        <MammograafDicomBerichtStatusView messageType={'DMWL'} level={this.getDmwlStatusLevel()} timestamp={mammograafStatus.laatsteSuccesDmwlBerichtTimestamp}
                                                          onClick={() => this.props.toonDicomFouten('DMWL', mammograafStatus)}/>
                    </Col>
                </Row>
                <Row className={'connectiestatus-leaf'}>
                    <Col>
                        <i className={connectieStatusLevels[this.getDatumStatusLevel()]}/>
                        <p>{this.getDatumStatusLevel() === 'OK' ? 'Datum mammograaf komt overeen met ScreenIT' :
                            this.getDatumStatusLevel() === 'FAULT' ? 'Datum mammograaf komt niet overeen met ScreenIT' :
                                'Datum mammograaf is nog niet ontvangen'}</p>
                    </Col>
                </Row>
            </Col>
        </Row>;
    ...
    getStatusLevel = (): ConnectieStatusLevel => {
        ...
        if (statusLevel !== this.props.mammograafConnectieStatusLevel) {
