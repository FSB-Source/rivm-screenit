...
export type OnderzoekBlokProps = {
    vorigOnderzoek: VorigOnderzoek
}

export default class OnderzoekBlokView extends Component<OnderzoekBlokProps> {
    emptyLezingenAanzichten: DoorsnedeAfbeeldingen;

    constructor(props: OnderzoekBlokProps) {
        super(props);
        this.props = props;
                    ...
                    <Row noGutters>
                        <Col md={3} className={'px-2'}>
                            <VisueleInspectieContainerView afspraakId={0}
                                                           ...
                                                           iconenById={this.props.vorigOnderzoek.visueleInspectieAfbeelding ?
                                                               this.props.vorigOnderzoek.visueleInspectieAfbeelding.iconenById :
                                                               new Map()}
                                                           paletIconen={[]}
                                                           noGutters={true} isEditable={false} amputatie={this.props.vorigOnderzoek.onderzoek.amputatie}/>
                        </Col>
                        <Col className={'px-2'} md={3}>
                            <AfwijkingenContainerView blokId={String(this.props.vorigOnderzoek.uitnodigingsNr)}
                                                      lezingAanzichten={this.props.vorigOnderzoek.signaleren && this.props.vorigOnderzoek.signaleren.doorsnedeAfbeeldingen ?
                                                          this.props.vorigOnderzoek.signaleren.doorsnedeAfbeeldingen :
                                                          this.emptyLezingenAanzichten}
                                                      afspraakId={0} isEditable={false} amputatie={this.props.vorigOnderzoek.onderzoek.amputatie}/>
                        </Col>
    ...
    maakOnderzoekRow = (label: string, property: string, index: number) => {
        return (
            <tr key={index}>
                <td>{label}</td>
