...
public class CervixMaakOrderBerichtServiceImpl implements CervixMaakOrderBerichtService
{
	...
	private DateTimeFormatter nu = DateTimeFormat.forPattern("yyyyMMddHHmmss");

	private DateTimeFormatter dateOnlyFormatter = DateTimeFormat.forPattern("dd-MM-yy");
	...
	{
		return maakOrderBericht(uitstrijkje, cytologieReden).toString();
	}

	@Override
	public OML_O21 maakOrderBericht(CervixUitstrijkje uitstrijkje, CervixCytologieReden cytologieReden)
	{
		...
		OML_O21 omlBericht = null;

		try
		{
			omlBericht = new OML_O21();
	...
	private void vulHeader(OML_O21 oml_bericht) throws DataTypeException
	{
		...
		mshSegment.getDateTimeOfMessage().getTimeOfAnEvent().setValue(nu.print(currentDateSupplier.getDateTime()));
	}
	...
	private void vulOrderRecords(CervixCytologieReden cytologieReden, CervixLabformulier labformulier, OML_O21 omlBericht) throws HL7Exception
	{
		...
		orc.getDateTimeOfTransaction().getTs1_TimeOfAnEvent().setValue(nu.print(currentDateSupplier.getDateTime()));
		...
		{
			makeNewObxRecord(omlBericht, DATUM_UITSTRIJKJE, dateOnlyFormatter.print(new DateTime(labformulier.getDatumUitstrijkje())));
		}
		...
		{
			makeNewObxRecord(omlBericht, LAATSTE_MENSTRUATIE, dateOnlyFormatter.print(new DateTime(datumLaatsteMenstruatie)));
		}
