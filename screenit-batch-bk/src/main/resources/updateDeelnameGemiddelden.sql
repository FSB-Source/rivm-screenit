...
with
    parameters as (
      select
        to_timestamp(:nu, 'yyyy-mm-ddThh24:mi:ss')           nu,
        :deelnamekansberekening_na_weken * interval '1 week' deelnamekansberekening_na_weken
  ),
    constanten as (
      select
        nu - deelnamekansberekening_na_weken deelnamekansberekening_tot,
        3                                   minimaal_aantal_voor_bepalen_gemiddelden
      from parameters
  ),

  -- Screeningronden waarvoor een sample moet worden aangemaakt
    screening_ronde_sample_aanmaken as (
      select
        sr.postcode          postcode,
        sr.standplaats_ronde standplaats_ronde
      from mamma.screening_ronde sr
        join mamma.uitnodiging u on sr.id = u.screening_ronde
        join mamma.brief b on u.brief = b.id
                              and b.gegenereerd
        left join mamma.merged_brieven mb on b.merged_brieven = mb.id
      where sr.screening_ronde_event is null
            and (
              mb.print_datum is not null and mb.print_datum < (select deelnamekansberekening_tot from constanten)
              or b.merged_brieven is null and b.creatie_datum < (select deelnamekansberekening_tot from constanten)
            )
            and sr.creatie_datum >= (select nu
                                     from parameters) - interval '5 year'
  ),
    postcodes_screening_ronde_sample_aanmaken as (
      select distinct postcode postcode
      from screening_ronde_sample_aanmaken
  ),
    postcode_cijfers_screening_ronde_sample_aanmaken as (
      select distinct left(postcode, 4) postcode_cijfer
      from postcodes_screening_ronde_sample_aanmaken
  ),
  ...
  -- basis query die alle benodigde info selecteerd
    basis as (
      select
        sr.creatie_datum                  creatie_datum,
        sr.postcode                       postcode,
        left(sr.postcode, 4)              postcode_cijfers,
        sr.standplaats_ronde              standplaats_ronde,
        b.id                              brief_id,
        b.merged_brieven                  brief_merged_brieven,
        b.creatie_datum                   brief_creatie_datum,
        mb.print_datum                    print_datum,
        a.onderzoek                       onderzoek,
        row_number() -- tbv groeperen op screening_ronde
        over (
          partition by sr.id
          order by a.onderzoek asc )      row_number_onderzoek,
      ...
      from mamma.dossier d
        ...
        join mamma.uitnodiging u on sr.id = u.screening_ronde
        left join mamma.brief b on u.brief = b.id
                                   and b.gegenereerd
        left join mamma.merged_brieven mb on b.merged_brieven = mb.id
        left join mamma.afspraak a on u.id = a.uitnodiging
                                      and a.onderzoek IS NOT NULL
  ),
  ...
  -- selectie
    selectie as (
      select *
      from basis
      where row_number_onderzoek = 1 -- groeperen op screening_ronde
            and (
              print_datum is not null and print_datum < (select deelnamekansberekening_tot from constanten)
              or brief_id is not null and brief_merged_brieven is null and brief_creatie_datum < (select deelnamekansberekening_tot from constanten)
            )
  ),
  ...
  --  creatie_datum
    landelijk_jaar10 as (
      select *
      from selectie
      where creatie_datum > (select nu from parameters) - interval '10 year'
  ),
    landelijk_jaar5 as (
      select *
      from landelijk_jaar10
      where creatie_datum > (select nu from parameters) - interval '5 year'
  ),
    landelijk_jaar3 as (
      select *
      from landelijk_jaar5
      where creatie_datum > (select nu from parameters) - interval '3 year'
  ),
  ...
  -- postcode
    postcode_jaar10 as (
      ...
      where postcode in (select postcode from postcodes_screening_ronde_sample_aanmaken)
  ),
    postcode_jaar5 as (
      select *
  ...
  -- Regio gemiddelden opslaan
    ...
    set
      ...
      deelname_afgelopen10jaar              = regio_gemiddelden.jaar10_gemiddelde,
      deelname_afgelopen5jaar               = regio_gemiddelden.jaar5_gemiddelde,
      deelname_afgelopen3jaar               = regio_gemiddelden.jaar3_gemiddelde,
      deelname_eerste_ronde_afgelopen10jaar = regio_gemiddelden.eerste_ronde_jaar10_gemiddelde,
      deelname_eerste_ronde_afgelopen5jaar  = regio_gemiddelden.eerste_ronde_jaar5_gemiddelde,
      deelname_eerste_ronde_afgelopen3jaar  = regio_gemiddelden.eerste_ronde_jaar3_gemiddelde
    from regio_gemiddelden
  ...
  -- Standplaatsronde gemiddelden opslaan
    ...
    set
      wijzigings_datum      = (select nu from parameters),
      deelname              = standplaats_ronde_gemiddelden.standplaats_ronde_gemiddelde,
      deelname_eerste_ronde = standplaats_ronde_gemiddelden.eerste_ronde_standplaats_ronde_gemiddelde
    from standplaats_ronde_gemiddelden
