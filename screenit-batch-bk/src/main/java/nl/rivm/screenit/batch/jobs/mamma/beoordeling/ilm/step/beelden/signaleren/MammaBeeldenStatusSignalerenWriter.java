...
public class MammaBeeldenStatusSignalerenWriter extends MammaAbstractBeeldenStatusSignalerenWriter<MammaMammografie>
{
	...
	protected void write(MammaMammografie mammografie)
	{
		...
		var accessionNumber = mammografie.getOnderzoek().getAfspraak().getUitnodiging().getScreeningRonde().getUitnodigingsNr();
		var statusDatum = mammografie.getIlmStatusDatum();
		var client = mammografie.getOnderzoek().getAfspraak().getUitnodiging().getScreeningRonde().getDossier().getClient();
