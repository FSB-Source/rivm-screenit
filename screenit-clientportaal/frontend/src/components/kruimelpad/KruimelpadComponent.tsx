...
const KruimelpadComponent = (props: KruimelpadComponentProps) => {
    ...
    const crumbs = routes
        .filter(route => matchPath(currentPath, {path: route.path}))
        .map(({path, ...routeProps}) => {
            const matchedPath = matchPath(currentPath, {path: path})
    ...
    return (
        <div className={classNames(styles.style, props.className)}>
            <Button label={"Terug"} lightStyle={true} displayArrow={ArrowType.ARROW_LEFT} arrowBeforeLabel={true} onClick={history.goBack} className={bvoStyles.light}/>
