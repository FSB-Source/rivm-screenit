...
select
  ...
  event.deelname                                             as event_deelname,
  event.is_suspect                                           as event_is_suspect
from mamma.kansberekening_screening_ronde_event event
  join mamma.screening_ronde as screening_ronde on event.id = screening_ronde.screening_ronde_event
  join mamma.standplaats_ronde as standplaats_ronde on screening_ronde.standplaats_ronde = standplaats_ronde.id
  ...
  left join mamma.kansberekening_regio_gemiddelden as postcode_cijfers_gem
    on postcode_cijfers_gem.regio_type = 'POSTCODE_CIJFERS' and postcode_cijfers_gem.regio like concat(left(screening_ronde.postcode, 4), '%%')
  left join mamma.kansberekening_regio_gemiddelden as postcode_gem on postcode_gem.regio_type = 'POSTCODE' and postcode_gem.regio = screening_ronde.postcode
...
where screening_ronde.creatie_datum >= now() - interval '3 year'
